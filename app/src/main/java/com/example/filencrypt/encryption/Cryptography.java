...
public class Cryptography {
    ...
    static Pair<Cipher, byte[]> newCipher(String passcode) {
        ByteArrayOutputStream header = new ByteArrayOutputStream();

        byte[] salt = new byte[SALT_LENGTH];
        RANDOM.nextBytes(salt);

        KeySpec keySpecification = new PBEKeySpec(passcode.toCharArray(), salt, ITERATION_MULTIPLIER * 1000, KEY_LENGTH);
    ...
    static Cipher getCipher(String passcode, InputStream inputStream) {
        try {
            ...
            KeySpec keySpecification = new PBEKeySpec(passcode.toCharArray(), salt, iterationCount, KEY_LENGTH);
